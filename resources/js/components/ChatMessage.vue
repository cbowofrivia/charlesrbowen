...
<template>
  <div
    ...
    :class="
      message.role === 'user' ? 'border-od-green/50' : 'border-od-blue/40'
    "
  >
    <span
      class="mb-1 flex items-center gap-1.5 text-xs"
      :class="message.role === 'user' ? 'text-od-green' : 'text-od-blue'"
    >
    ...
    <div
      ...
      class="text-[0.8125rem] leading-[1.4] text-od-bright"
    >
      {{ message.content }}
    </div>
    ...
    <template v-else>
      ...
      />
      <span v-else class="inline-block h-4 w-2 animate-pulse bg-od-blue" />
    </template>
  </div>
</template>
